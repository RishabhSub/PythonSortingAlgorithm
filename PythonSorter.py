...
# Creating a class called DrawInformation
...
# Creates a random list given the size, minimum value, and maximum value of the list
def generate_start_list(n, min_value, max_value):
    lst = []

    for _ in range(n):
        val = random.randint(min_value, max_value) # min and max inclusive
        lst.append(val) # adds the value "val" to the end of the list "lst"

    return lst

# Prints a legend on the visualizer's head, and draws the rectangles
def draw(draw_info, algo_name, ascending):
    ...
    # basically writing the code for the legend and the placement of the legend
...
# Drawing the list of blocks for the initial unsorted block
def draw_list(draw_info, color_position = {}, clear_background = False):
    ...
    for i, val in enumerate(lst):
        ...
        # every three rectangles, the grey gradient will repeat
        color = draw_info.GRADIENTS[i % 3]

        # finds the color from the dictionary using the key : value pairing 
    ...
    for i in range(len(lst) - 1):
        for j in range(len(lst) - 1 - i):
            ...
            if (num1 > num2 and ascending) or (num1 < num2 and not ascending):
                ...
                yield True 
                # yielding instead of returning creates a generator object
                # this function "bubble_sort" is now a generator instead of a function
